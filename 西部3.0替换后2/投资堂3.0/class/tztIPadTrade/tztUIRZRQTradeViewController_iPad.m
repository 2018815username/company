 ...
 *
 ***************************************************************/
#import "tztUIRZRQTradeViewController_iPad.h"
...
@implementation tztUIRZRQTradeViewController_iPad
...
-(void)LoadLayoutView
{
    ...
    NSString* strPath = GetPathWithListName(@"tztRZRQTradeStockSetting",NO);
    NSDictionary* listvalue = [[[NSDictionary alloc] initWithContentsOfFile:strPath] autorelease];
    if (listvalue == NULL || [listvalue count] <= 0)
    {
        strPath = GetTztBundlePlistPath(@"tztRZRQTradeStockSetting");
    ...
    [_tztBaseView bringSubviewToFront:_titleSegment];
    ...
    if (_pTableView == NULL)
    {
        ...
        NSString* Plistfile = @"tztUITradeRZRQListSetting";
        if (_titleSegment)
        {
...
-(void)NsKeyTypeFirstView
{
    ...
    switch (self.nKeyType)
    {
        ...
        {
            if (_pTableView)
                [_pTableView SetMsgType:WT_JJINQUIREGUFEN];
        }
            break;
        case MENU_QS_HTSC_ZJLC_List:
        {
            if (_pTableView)
                [_pTableView SetMsgType:MENU_QS_HTSC_ZJLC_QueryStock];
        }
            break;
        case WT_JYRZRQ:
        {
            if (_pTableView)
                [_pTableView SetMsgType:WT_RZRQQUERYGP];
        }
            break;
        default:
            break;
...
//                }
//            }else 
            if ([strID intValue] == WT_RZRQOut)//修改融资融券退出 modify by xyt 20131028
            {
...
//            }
//            else 
            if ([strID intValue] == WT_RZRQOut)//清空界面数据  //修改融资融券退出 modify by xyt 20131028
            {
                _bTradeOut = TRUE;
                [TZTUIBaseVCMsg OnMsg:[strID intValue] wParam:0 lParam:0];
            }else
...
-(void)SetNsKeyType:(int)KeyType
{
    if(_pRightTradeViews && [_pRightTradeViews count] > 0 && self.nKeyType > 0)
    {
        ...
        {
            CGRect rcContent = _tztBounds;// self.view.bounds;
            rcContent.origin.y +=  _titleSegment.frame.origin.y +_titleSegment.frame.size.height;
            rcContent.origin.x += _pTableView.frame.size.width;
            rcContent.size.width -= _pTableView.frame.size.width;
            rcContent.size.height -= rcContent.origin.y;
