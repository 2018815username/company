...
-(void)SetDefaultData
{
    ...
    NSDate *endDate = [[NSDate date] dateByAddingTimeInterval:( 10.0 * 365 * 24 * 60 *60)];//结束日期为开始日期往后10年
...
//                }
                ...
                {
                    NSString* strJJSF = [ayData objectAtIndex:JJSFFSINDEX];
                    if (_tztTradeTable)
...
-(BOOL)CheckInput
{
    ...
    else
    {
        ...
        strInfo = [NSString stringWithFormat:@"基金代码:%@\r\n基金名称:%@\r\n扣款周期:%@\r\n扣款日期:%@\r\n开始日期:%@\r\n结束日期:%@\r\n定投金额:%@\r\n 确认定投？",nsCode, nsName,nsZQ,nsKKRQ,nsBegin,nsEnd,nsMoney];
    }
...
-(void)OnRisk
{
    ...
    pVC.nHasToolbar = 0;
    NSString* strFile = [[g_pSystermConfig.pDict tztObjectForKey:tztSystermConfig_FundParams] tztObjectForKey:tztSystermConfig_FundRiskURL];
    if (strFile.length > 0)
    {
...
-(void)TZTUIMessageBox:(TZTUIMessageBox *)TZTUIMessageBox clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0)
    {
        switch (TZTUIMessageBox.tag)
        {
            case 0x1111:
            {
                int nNeedCheckRisk = 0;
                NSString* strType = [[g_pSystermConfig.pDict tztObjectForKey:tztSystermConfig_FundParams] tztObjectForKey:tztSystermConfig_FundDTNeedRiskCheck];
                if (strType && strType.length > 0)
                    nNeedCheckRisk = [strType intValue];
                if (nNeedCheckRisk > 0)
                {
