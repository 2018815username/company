...
@property(nonatomic,retain)tztUIStockInfoView    *pInfoView;
@end
...
-(void)setFrame:(CGRect)frame
{
    ...
    if (_pInfoView == NULL)
    {
        self.pInfoView = [[tztUIStockInfoView alloc] initWithFrame:self.bounds];
    }
...
//        tztUITrendView_iphone *pTrend = [[tztUITrendView_iphone alloc] initWithFrame:self.bounds];
...
//        tztUIStockInfoView *pInfo = [[tztUIStockInfoView alloc] initWithFrame:self.bounds];
        if (self.pInfoView && [self.pInfoView respondsToSelector:@selector(setStockInfo:Request:)])
        {
            [self.pInfoView setStockInfo:self.pStockInfo Request:FALSE];
        }
        if (self.pInfoView != nil)  // Avoid potential leak.  byDBQ20131031
        {
