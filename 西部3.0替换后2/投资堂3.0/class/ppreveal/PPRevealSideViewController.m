...
NSInteger   g_nSupportLeftSide = 0;
NSInteger   g_nSupportRightSide = 0;

#pragma mark - Unit constants
//static const CGFloat DefaultOffset = 30.0;
...
@synthesize wasClosed = _wasClosed;
@synthesize IgnoredTouch = _IgnoredTouch;
...
- (void)pushViewController:(UIViewController *)controller onDirection:(PPRevealSideDirection)direction animated:(BOOL)animated completion:(void (^)())completionBlock {
    ...
    if (direction == PPRevealSideDirectionLeft)
        offset = tzt_SideWidthLeft;
    else if (direction == PPRevealSideDirectionRight)
        offset = tzt_SideWidthRight;
    [self pushViewController:controller onDirection:direction withOffset:offset animated:animated completion:completionBlock];
}
...
- (void)pushViewController:(UIViewController *)controller onDirection:(PPRevealSideDirection)direction animated:(BOOL)animated forceToPopPush:(BOOL)forcePopPush completion:(void (^)())completionBlock {
    ...
    [self pushViewController:controller onDirection:direction withOffset:offset animated:animated forceToPopPush:forcePopPush completion:completionBlock];
}
...
- (void)replaceAfterOpenedCompletelyAnimated:(BOOL)animated completion:(void (^)())completionBlock {
    ...
    if (_currentPanDirection == PPRevealSideDirectionLeft)
        ...
        offset = tzt_SideWidthRight;
    [self replaceAfterOpenedCompletelyWithOffset:offset animated:animated completion:completionBlock];
}
...
- (void)preloadViewController:(UIViewController *)controller forSide:(PPRevealSideDirection)direction withOffset:(CGFloat)offset forceRemoval:(BOOL)force {
    ...
    if (existingController != controller) {
        ...
        BOOL bIsViewLoaded = [controller isViewLoaded];
        if (controller && [controller isKindOfClass:[UINavigationController class]])
        {
            bIsViewLoaded = ((UINavigationController*)controller).topViewController.isViewLoaded;
        }
        
        if (/*![controller isViewLoaded]*/!bIsViewLoaded)
        {
...
//    _rootViewController.view.layer.shadowColor = [UIColor colorWithTztRGBStr:@"36,36,36"].CGColor;//[UIColor colorWithPatternImage:[UIImage imageTztNamed:@"TZTTabBarBG.png"]].CGColor;
...
- (void)addPanGestureToController:(UIViewController *)controller {
    ...
    [self addCustomsViews];
}

-(void)addCustomsViews
{
    // Customs views
...
//    return [_rootViewController shouldAutorotateToInterfaceOrientation:toInterfaceOrientation];
    ...
    {
        return [[g_navigationController topViewController] shouldAutorotateToInterfaceOrientation:toInterfaceOrientation];
    }
