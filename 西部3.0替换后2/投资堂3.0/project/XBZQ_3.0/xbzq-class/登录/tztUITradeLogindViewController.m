...
tztUITradeLogindViewController *g_loginVC;
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    {
#ifdef SaveAccountInLocal
        [_pTradeLoginView refreshWhenAppear];
#endif
    }
    g_loginVC = self;
}
...
-(void)LoadLayoutView
{
    ...
    else
    {
        ...
        [self onSetTztTitleView:((_nLoginType == TZTAccountRZRQType) ? @"融资融券登录" : @"交易登录") type:TZTTitleNormal];
#else
        [self onSetTztTitleView:((_nLoginType == TZTAccountRZRQType) ? @"融资融券登录" : @"交易登录") type:TZTTitleReturn|TZTTitleNormal];
#endif
#else
    ...
    if (_pTradeLoginView == nil)
    {
        ...
        [_pTradeLoginView setMsgID:_nMsgID MsgInfo:self.pMsgInfo LPARAM:_lParam];
