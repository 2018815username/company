...
//    //添加转让类型ruyi
//    UIView* zrlx = [_tztTradeView getCellWithFlag:@"TZTZRLX"];
//    [zrlx.subviews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
//        NSLog(@"%@",obj);
//        if (idx ==2 && [obj isKindOfClass:[tztUILabel class]]) {
//            tztUILabel* label = (tztUILabel*)obj;
//            [label setLabelBackgroundColor:@"255,0,0" andMyBackGroundImage:nil];
//        }
//    }];
}
...
-(void) ClearData
{
    ...
    [_tztTradeView setEditorText:@"" nsPlaceholder_:@"" withTag_:2005];
    self.CurStockCode = @"";
    [self requestTrend];
}
...
//                [_tztTradeView setLabelText:@"-" withTag_:kTagStockNew];
    ...
    if ([pParse IsAction:@"5124"]) //获取股转类型数据
    {
        ...
        if ([[pParse GetErrorMessage]isEqualToString:@"查无记录!"] || [pParse GetErrorNo] < 0)
        {
