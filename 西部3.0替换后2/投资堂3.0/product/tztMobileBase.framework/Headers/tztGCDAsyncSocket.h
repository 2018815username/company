 ...
 * 文件标识：
 * 摘    要：GCD通讯库 加入了OpenSSL支持 进行了简化，只支持线程通讯
 *
 * 当前版本：
 * 作    者：yangdl
 * 完成日期：2012.02.29
 ...
 *
 * 修改记录：
 * 原始来源：https://github.com/robbiehanson/CocoaAsyncSocket
 *******************************************************************************/
...
@class tztGCDAsyncReadPacket;
@class tztGCDAsyncWritePacket;
@class tztGCDAsyncSocketPreBuffer;

#if TARGET_OS_IPHONE
     // iOS 5.0 not supported
#else
    // Compiling for Mac OS X
#endif

extern NSString *const tztGCDAsyncSocketException;
extern NSString *const tztGCDAsyncSocketErrorDomain;

extern NSString *const tztGCDAsyncSocketQueueName;
extern NSString *const tztGCDAsyncSocketThreadName;

enum tztGCDAsyncSocketError
{
	tztGCDAsyncSocketNoError = 0,           // Never used
	tztGCDAsyncSocketBadConfigError,        // Invalid configuration
	tztGCDAsyncSocketBadParamError,         // Invalid parameter was passed
	tztGCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	tztGCDAsyncSocketReadTimeoutError,      // A read operation timed out
	tztGCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	tztGCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	tztGCDAsyncSocketClosedError,           // The remote peer closed the connection
	tztGCDAsyncSocketOtherError,            // Description provided in userInfo
};
typedef enum tztGCDAsyncSocketError tztGCDAsyncSocketError;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface tztGCDAsyncSocket : NSObject
{
	uint32_t flags;
	uint16_t config;
	...
	tztGCDAsyncReadPacket *currentRead;
	tztGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	tztGCDAsyncSocketPreBuffer *preBuffer;
    NSString* _hostName;
    uint16_t _hostPort;
...
@protocol tztGCDAsyncSocketDelegate
@optional
...
**/
- (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(tztGCDAsyncSocket *)sock;
...
**/
- (void)socket:(tztGCDAsyncSocket *)sock didAcceptNewSocket:(tztGCDAsyncSocket *)newSocket;
...
**/
- (void)socket:(tztGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(tztGCDAsyncSocket *)sock didWriteDataOfLength:(NSUInteger)partialLength tag:(long)tag;//didWriteDataWithTag:(long)tag;
...
**/
- (NSTimeInterval)socket:(tztGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(tztGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(tztGCDAsyncSocket *)sock;
...
**/
- (void)socketDidSecure:(tztGCDAsyncSocket *)sock;
