...
enum
{
    kTagStockCode = 999,  //可输可选控件，国联开始用到
...
-(void) ClearData
{
    if (_pCrashReturn == NULL)
        return;
    [_pCrashReturn setComBoxData:nil ayContent_:nil AndIndex_:-1 withTag_:kTagStockCode];
    ...
    [_pCrashReturn setLabelText:@"" withTag_:1004];
}
...
//        return FALSE;
    ...
    //股票代码
    NSString* nsCode = @"";
    if ([_pCrashReturn getViewWithTag:KTag_StockCode])
    {
        nsCode = [_pCrashReturn GetEidtorText:KTag_StockCode];
    }
    else
    {
...
-(void)OnRequestSendData
{
    ...
    [[tztMoblieStockComm getShareInstance] onSendDataAction:@"418" withDictValue:pDict];
    DelObject(pDict);
}
...
- (void)tztDroplistView:(tztUIDroplistView *)droplistview didSelectIndex:(int)index
{
    if ([droplistview.tzttagcode intValue] == kTagStockCode)
    {
        ...
        if (rangeLeft.location > 0 && rangeLeft.location < [strTitle length])
        {
            ...
            //设置股票代码
            [_pCrashReturn setComBoxText:strCode withTag_:kTagStockCode];
...
//        [self OnInquireFund];
//#endif
        [_pCrashReturn setLabelText:[_ayStockNum objectAtIndex:0] withTag_:KTag_MaxReturn];
    }
    ...
    if ([pParse IsAction:@"418"])//查询可还券
    {
        ...
        if (_pCrashReturn && [pAyTitle count] > 0)
        {
            [_pCrashReturn setComBoxData:pAyTitle ayContent_:pAyTitle AndIndex_:-1 withTag_:kTagStockCode bDrop_:YES];
        }
