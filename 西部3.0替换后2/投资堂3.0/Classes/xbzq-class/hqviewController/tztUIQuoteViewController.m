...
//    _pTitle.backgroundColor = [UIColor redColor];
}
...
-(void)LoadLayoutViewAnimated
{
  ...
  //  [_pTitle setSelectSegmentIndex:1];
    ...
    rcNine.size.height = KgridHeight;
...
//    _pIndexTrendScroll.backgroundColor = [UIColor redColor];//wry
    if (_nReportType == tztReportDAPANIndex)
    {
...
//        [_tztBaseView bringSubviewToFront:_pIndexTrendScroll];
    ...
    }else if (_nSegIndex ==1) {
        rcList.size.height -= (rcMarket.origin.y + rcMarket.size.height)+(KgridHeight);
    }else{
         rcList.size.height -= (rcMarket.origin.y + rcMarket.size.height);
    }
    if (_pReportList == nil)
...
//    _pNineGridView.hidden =NO;
    if (!_pNineGridView.hidden) {
        _pNineGridView.frame = CGRectMake(0, TZTScreenHeight-KgridHeight-49,TZTScreenWidth, KgridHeight);
    }else{
    _pNineGridView.frame = CGRectMake(0, TZTScreenHeight-49,TZTScreenWidth, KgridHeight);
    }
...
//在这里设置标题的 的市场和自选    
   ...
   rcFrame.origin.y += (_pTitle.frame.size.height + _pTitle.frame.origin.y); //(0,44,320,411)
    ...
    if (_pNineGridView == NULL)
    {
        ...
        _pNineGridView.frame =  CGRectMake(0, TZTScreenHeight-49-65, TZTScreenWidth, 65);
...
//    [pSwitch setTztTitleColor:[UIColor tztThemeHQBalanceColor]];
    pSwitch.pNormalColor = [UIColor whiteColor];
    pSwitch.pUnCheckedColor = [UIColor whiteColor];
//    [pSwitch setMiddleSeperator:[UIImage imageNamed:@"select"]];
    pSwitch.yesImage = [UIImage imageNamed:@"select"];
    pSwitch.noImage = [UIImage imageNamed:@"select"];
    pSwitch.bUnderLine = YES;
