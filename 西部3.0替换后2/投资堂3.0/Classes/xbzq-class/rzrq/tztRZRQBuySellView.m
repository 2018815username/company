...
-(void)OnInquireFund
{
    ...
    //增加账号类型获取token
    [pDict setTztObject:[NSString stringWithFormat:@"%ld",(long)TZTAccountRZRQType] forKey:tztTokenType];
    switch (_nMsgType) 
    {
...
//                NSString* nsDefault = [NSString stringWithFormat:@"%@(%@)", nsCode, nsName];
    ...
    if ([pParse IsAction:@"406"])
    {
        ...
        int ndbpjeindex = -1;//担保品可用金额
        
        NSString *strIndex = [pParse GetByName:@"fzzjeindex"];
        TZTStringToIndex(strIndex, nFzzjeindex);
        
        strIndex = [pParse GetByName:@"AssureEnbuyBalance"];
        TZTStringToIndex(strIndex, ndbpjeindex);
        if (ndbpjeindex<0) {
            strIndex = [pParse GetByName:@"USABLEINDEX"];
            TZTStringToIndex(strIndex, ndbpjeindex);
        }
        ...
        if (ayGrid)
        {
            for (int i = 1; i < [ayGrid count]; i++)
            {
                ...
                if (_nMsgType ==  WT_RZRQBUY || _nMsgType == MENU_JY_RZRQ_PTBuy
                    || _nMsgType == WT_RZRQSALE || _nMsgType == MENU_JY_RZRQ_PTSell)
                {
                    if (ndbpjeindex >= 0 && [ayData count] > ndbpjeindex)
                    {
                        NSString *str = [ayData objectAtIndex:ndbpjeindex];
