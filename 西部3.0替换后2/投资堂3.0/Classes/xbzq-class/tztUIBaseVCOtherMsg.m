...
//                                         target:self
//                                       selector:@selector(CheckSysAndTradeLogin:wParam:lParam:)
//                                       userInfo:nil
//                                        repeats:NO];
    ...
    switch (nMsgType)
    {
     ...
     //查询持仓xbxty/app/jy/jy_cxcc.htm
        ...
        case MENU_SYS_ReLogin:
        {
            ...
            //clearAllSaveAccount
            [[tztUserData getShareClass] clearAllSaveAccount];
        ...
        case HQ_MENU_TztPushInfo://推送详细信息
        {
            ...
            if(strURL == nil || [strURL length] <= 0)
                return TRUE;
            NSString* strUrla = [NSString stringWithFormat:strURL,[ayPush objectAtIndex:0],[ayPush objectAtIndex:1]];
...
//                                            repeats:NO];
        ...
        case MENU_SYS_JYLogout:
        {
            ...
            NSString* strContext = [[pDict tztObjectForKey:@"context"] tztdecodeURLString];
...
//            NSString *strUrlSecond = @"/yjb/warnList.html";
          ...
          NSString*mobile =   [tztKeyChain load:tztLogMobile];
            if (mobile.length<11) {
                //系统登录
                tztUISysLoginViewController* pVC = [[tztUISysLoginViewController alloc] init];
                [pVC setMsgID:nMsgType MsgInfo:(void*)wParam LPARAM:lParam];
            ...
            pVC.nMsgType = nMsgType;
            [pVC setTitle:@"股价提醒"];
            NSString* strFile = @"xbxty/app/msg/warn_set.html";
...
//    [NSThread sleepForTimeInterval:0.2];
    ...
    {
// wry 现在代码里面有一个循环跳转的问题 -- 先看钥匙串里面有没有手机号 然后再看对象里面有没有
        ...
        else if (g_CurUserData.nsMobileCode && g_CurUserData.nsMobileCode.length >=11 && ![TZTUserInfoDeal IsHaveTradeLogin:Systerm_Log])
        {
            strLogMobile = [tztUserData getShareClass].nsMobileCode;
...
//        || (nMsgType == HQ_MENU_MergeUserStock)     //合并自选))//普通交易。基金
        || (nMsgType == MENU_JY_PT_XinGuShenGou)
        ) // 新股申购
//        || (nMsgType == TZT_MENU_OpenWebInfoContent)//打开新的网页 wry
        //         || (nMsgType == MENU_SYS_UserWarning)
