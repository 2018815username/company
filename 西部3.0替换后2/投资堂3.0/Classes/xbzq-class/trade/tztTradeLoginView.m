...
-(void)setLoginOtherViewe{
    ...
    _loginBtn.frame = CGRectMake(20, 44*topHeightLineNumber+40, TZTScreenWidth-40, 40);
    [_tztTableView addSubview:_loginBtn];
    
    
    _forgetPassWorld =[UIButton buttonWithType:UIButtonTypeCustom];
    _forgetPassWorld.backgroundColor =[UIColor clearColor];
    _forgetPassWorld.titleLabel.textAlignment = UITextAlignmentRight;
    _forgetPassWorld.titleLabel.font = [UIFont systemFontOfSize: 14];
...
//    _forgetPassWorld.layer.borderColor = [UIColor colorWithRed:240.0/255.0 green:241.0/255 blue:245.0/255.0 alpha:1].CGColor;
    [_forgetPassWorld setTitleColor:[UIColor colorWithRed:62.0/255.0 green:76.0/255 blue:198.0/255.0 alpha:1] forState:UIControlStateNormal];
    ...
    _forgetPassWorld.frame = CGRectMake(20+(TZTScreenWidth-40)/2+55, top+10, (TZTScreenWidth-40)/2-20, 30);
    [_tztTableView addSubview:_forgetPassWorld];//forgetPwd
    
    UILabel *linelable = [[UILabel alloc] initWithFrame:CGRectMake(20+(TZTScreenWidth-40)/2+58+25, top+11+23, (TZTScreenWidth-40)/2-20-60, 1)];
    linelable.backgroundColor=[UIColor colorWithRed:62.0/255.0 green:76.0/255 blue:198.0/255.0 alpha:1];
    [_tztTableView addSubview:linelable];
    ...
    [_kaihu setBackgroundImage:[UIImage imageNamed:@"tztkh.png"] forState:UIControlStateNormal];
    [_kaihu addTarget:self action:@selector(kaiHuBtnClick) forControlEvents:UIControlEventTouchUpInside];
    _kaihu.frame = CGRectMake(0, _tztTableView.frame.size.height-100, TZTScreenWidth, 100);
...
-(void)kaiHuBtnClick
{
    NSURL *url = [NSURL URLWithString:@"crhsjkh://"];
    if ([[UIApplication sharedApplication] canOpenURL:url]){
        tztAfxMessageBlock(@"您已安装手机开户软件,是否确认跳转", @"系统提示",nil, TZTBoxTypeButtonBoth, ^(NSInteger index) {
            if (index ==0) {
                NSURL *url = [NSURL URLWithString:@"crhsjkh://"];
        ...
        tztAfxMessageBlock(@"您尚未安装掌上开户软件,是否确认跳转AppStore", @"系统提示",nil, TZTBoxTypeButtonBoth, ^(NSInteger index) {
            if (index ==0) {
...
//    if ([[UIApplication sharedApplication] canOpenURL:url]) {
//
//        UIAlertView*alter = [[UIAlertView alloc] initWithTitle:@"系统提示" message:@"您已安装手机开户软件,是否确认跳转" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
//        alter.tag =100;
//        [alter show];
//        
//    }else{
...
////     [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://itunes.apple.com/cn/app/xi-bu-zheng-quan-zhang-shang/id962278472?l=en&mt=8"]];
//
//        UIAlertView*alter = [[UIAlertView alloc] initWithTitle:@"系统提示" message:@"您尚未安装掌上开户软件,是否确认跳转AppStore" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
//        alter.tag =101;
//        [alter show];
//    }
}
...
-(void)OnLogin
{
    ...
    NSString* nsAccountType = @"ZJACCOUNT";//委托类别
